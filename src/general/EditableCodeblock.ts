...
export class EditableCodeblock {
	...
	render(): void {
		switch (this.settings.renderMode) {
			case 'textarea':
				...
				break;
			case 'pre':
				void this.emit_render(this.el).then(() => {
					if (this.isReadingMode || this.isMarkdownRendered) {
						if (typeof (this as any).ensureCopyButton === 'function') {
							try {
								(this as any).ensureCopyButton(this.el, this.outerInfo.source ?? this.innerInfo.source_old);
							} catch (error) {
								// noop
							}
						}
	...
	private renderTextareaPre(): void {
		...
		div.classList.add('editable-codeblock', 'editable-textarea');

		// 应用主题检测（检查是否有主题检测方法）
		if (typeof (this as any).applyThemeClass === 'function') {
			try {
				// console.log('开始应用主题检测...');
				(this as any).applyThemeClass(div);
				(this as any).observeThemeChanges(div);
				// console.log('主题检测应用完成');
			} catch (error) {
				console.log('主题检测失败:', error);
			}
		} else {
			// console.log('没有主题检测方法，跳过主题检测');
		}
		...
		editEl.classList.add('language-edit');
		editEl.setAttribute('align', 'right');
		editInput = document.createElement('input');
		editEl.appendChild(editInput);
		...
		if (this.isReadingMode || this.isMarkdownRendered) {
			...
			// show copy button in reading mode
			if (typeof (this as any).ensureCopyButton === 'function') {
				try {
					(this as any).ensureCopyButton(div, this.outerInfo.source ?? this.innerInfo.source_old);
				} catch (error) {
					// noop
				}
			}
		...
		if (this.settings.saveMode == 'onchange') {
			...
			textarea.onchange = (ev): void => {
				// save must on oninput: avoid: textarea --update--> source update --update--> textarea (lose curosr position)
		...
		if (editInput) {
			...
			}
			{
				...
				editInput.onchange = (ev): void => {
					...
					this.outerInfo.language_meta = match[2];
					div.classList.remove('is-no-saved');
					void this.emit_save(true, false);
				};
			}

			this.enable_editarea_listener(
				editInput,
				undefined,
				ev => {
					ev.preventDefault(); // safe: tested: `prevent` can still trigger `onChange
	...
	private async renderEditablePre(): Promise<void> {
		...
		const div = document.createElement('div');
		this.el.appendChild(div);
		div.classList.add('editable-codeblock', 'editable-pre');
		...
		if (!pre || !code) {
			LLOG.error("render failed. can't find pre/code 1");
			return;
		}
		...
		if (this.settings.saveMode == 'onchange') {
			void Promise.resolve().then(() => {
				if (!global_refresh_cache) return;
				if (!pre || !code) {
					LLOG.error("render failed. can't find pre/code 11");
					global_refresh_cache = null;
					return;
				}
			...
			code.addEventListener('blur', (ev): void => {
				// save must on oninput: avoid: textarea --update--> source update --update--> textarea (lose curosr position)
		...
		// refresh/save strategy2: cache and rebuild
			...
			void Promise.resolve().then(() => {
				if (!global_refresh_cache) return;
				if (!pre || !code) {
					LLOG.error("render failed. can't find pre/code 21");
					global_refresh_cache = null;
					return;
				}
				this.renderEditablePre_restoreCursorPosition(pre, global_refresh_cache.start, global_refresh_cache.end);
		...
		const emit_change = (newValue: string, isRender: boolean, isSave: boolean, isSavePos: boolean): void => {
			if (isComposing) return;
			if (!pre || !code) {
				LLOG.error("render failed. can't find pre/code 12");
				return;
			}
			...
			if (isRender) {
				void Promise.resolve().then(async () => {
					// like vue nextTick, ensure that the cursor is behind
					pre = div.querySelector(':scope>pre');
					code = div.querySelector(':scope>pre>code');
					if (!pre || !code) {
						LLOG.error("render failed. can't find pre/code 13");
						global_refresh_cache = null;
						return;
					}
	...
	private renderEditablePre_restoreCursorPosition(container: Node, start: number, end: number): void {
		...
		function traverse(node: Node): void {
			if (node.nodeType === Node.TEXT_NODE) {
				// pre/code is Node.ELEMENT_NODE, not inconformity
				const nextIndex = charIndex + (node.nodeValue?.length ?? 0);
				if (!isFoundStart && start >= charIndex && start <= nextIndex) {
					// start
					range.setStart(node, start - charIndex);
					isFoundStart = true;
				}
				if (isFoundStart && !isFoundEnd && end >= charIndex && end <= nextIndex) {
					// end
	...
	protected enable_editarea_listener(
		el: HTMLElement,
	...
	async emit_render(targetEl: HTMLElement): Promise<void> {
		...
		if (this.settings.renderEngine == 'shiki') {
			...
			for (const item of bundledThemesInfo) {
				if (item.id == this.settings.theme) {
			...
			if (!code) {
				...
				if (!codeElement) {
					LLOG.error('shiki return preStr without code tag', doc);
					return;
				}
		...
		// pre html string - prism, insert `<pre>...<pre/>`
			...
			if (!code) {
				...
				code = document.createElement('code') as HTMLPreElement;
				pre.appendChild(code);
				code.classList.add('language-' + this.outerInfo.language_type);
